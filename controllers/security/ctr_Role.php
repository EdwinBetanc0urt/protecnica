 ...
 */
function fcUltimoCodigoRol()
{
	$objInstancia = new clsRol(); //instancia la clase
...
function fcRegistrarRol()
{
	global $gsIndex; // variable global que contiene la ubicación del header
	$objInstancia = new clsRol(); //nuevo objInstancia o clase Presentación
		...
		if ($arrConsulta[$objInstancia->atrEstatus] == "inactivo") {
			$arrRetorno = array(
				...
				"datos" => array(
					...
					"idrol" => $arrConsulta["rol_superior_fk"],
...
function fcModificarRol()
{
	global $gsIndex;
	$objInstancia = new clsRol();
...
function fcListaRol()
{
	...
	$rstRecordSet = $objInstancia->fmListarIndexRol();
