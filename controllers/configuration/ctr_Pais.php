 ...
 */
function fcUltimoCodigoPais()
{
	$objInstancia = new clsPais(); //instancia la clase
...
function fcRegistrarPais()
{
	global $gsIndex; // variable global que contiene la ubicación del header
	$objInstancia = new clsPais(); //nuevo objInstancia o clase Presentación
		...
		if ($arrConsulta[$objInstancia->atrEstatus] == "inactivo") {
			$arrRetorno = array(
				...
				"datos" => array(
					...
					"cod_iso_numerico" => $arrConsulta["cod_iso_3166-1_numerico"],
					"cod_iso_alfa_2" => $arrConsulta["cod_iso_3166-1_alfa-2"],
					"cod_iso_alfa_3" => $arrConsulta["cod_iso_3166-1_alfa-3"],
					"prefijo_telefonico" => $arrConsulta["prefijo_telefonico"],
					"ciudad_capital" => $arrConsulta["ciudad_capital_pais_fk"],
...
function fcModificarPais()
{
	global $gsIndex;
	$objInstancia = new clsPais();
...
//Función Combo Dinámico País
//Utilizada por la vista de ACCESO ROL
function fcComboPais()
{
...
function fcListaPais()
{
	...
	$rstRecordSet = $objInstancia->fmListarIndexPais();
				...
				<tbody>
					<?php while ($arrRegistro = $objInstancia->getConsultaAsociativo($rstRecordSet)) : ?>
						<tr onclick='fjSeleccionarRegistro(this);' data-toggle='tooltip' data-placement='top' title='Doble clic para detallar los datos y realizar alguna operación'
							...
							|<?= $arrRegistro["cod_iso_3166-1_numerico"]; ?>
							|<?= $arrRegistro["cod_iso_3166-1_alfa-2"]; ?>
							|<?= $arrRegistro["cod_iso_3166-1_alfa-3"]; ?>
							|<?= $arrRegistro["prefijo_telefonico"]; ?>
							|<?= $arrRegistro["ciudad_capital_pais_fk"]; ?>' >
