...
if (is_file("models" . DS . $gsModulo . DS . "cls_{$gsComponente}.php")) {
	$gsRutaBase = "";
}
elseif (is_file(".." . DS . "models" . DS . $gsModulo . DS . "cls_{$gsComponente}.php")) {
	$gsRutaBase = ".." . DS;
}
else {
	$gsRutaBase = ".." . DS . ".." . DS;
}
require_once($gsRutaBase . "models" . DS . $gsModulo . DS . "cls_{$gsComponente}.php");
 ...
 */
function fcUltimoCodigoBoton()
{
	$objInstancia = new clsBoton(); //instancia la clase
...
function fcRegistrarBoton()
{
	$objInstancia = new clsBoton(); //nuevo objInstancia o clase Presentación
...
function fcModificarBoton()
{
	$objInstancia = new clsBoton();
...
//Función Combo Dinámico Botón
//Utilizada por la vista de ACCESO ROL
function fcComboBoton()
{
...
function fcListaBoton()
{
	global $gsComponente, $gsModulo; //variable que contiene la cadena con el nombre de la Clase u Objeto
	$objInstancia = new clsBoton; //instancia la clase
	...
	$rstRecordSet = $objInstancia->fmListarIndexBoton();
	...
	if ($rstRecordSet) {
		...
		<div class='table-responsiveX'>
			<br />
			<table border='0' valign='center' class='table table-striped text-center table-hover table-condensed table-bordered' id="tabLista<?= $gsComponente; ?>">
				<thead>
					<tr>
						<th datos_orden_metodo="asc" datos_orden="<?= $objInstancia->atrId_P; ?>" onclick='fjMostrarLista("<?= $gsModulo; ?>", "<?= $gsComponente; ?>", "<?= $vpPaginaActual; ?>", "<?= $objInstancia->atrId_P; ?>")' >
							Cod
							<span class='glyphicon glyphicon-sort-by-attributes'></span>
						</th>
						<th datos_orden_metodo="asc" datos_orden="<?= $objInstancia->atrNombre; ?>" onclick='fjMostrarLista("<?= $gsModulo; ?>", "<?= $gsComponente; ?>", "<?= $vpPaginaActual; ?>", "<?= $objInstancia->atrNombre; ?>")' >
							Descripción
							<span class='glyphicon glyphicon-sort'></span>
						</th>
						<th datos_orden_metodo="asc" datos_orden="<?= $objInstancia->atrEstatus; ?>" onclick='fjMostrarLista("<?= $gsModulo; ?>", "<?= $gsComponente; ?>", "<?= $vpPaginaActual; ?>", "<?= $objInstancia->atrEstatus; ?>")' >
							Estatus
							<span class='glyphicon glyphicon-sort'></span>
						</th>
					</tr>
				...
				<tbody>
					<?php while ($arrRegistro = $objInstancia->getConsultaAsociativo($rstRecordSet)) : ?>
						<tr onclick='fjSeleccionarRegistro(this);' data-toggle='tooltip'
							data-placement='top' title='Doble clic para detallar los datos y realizar alguna operación'
		...
		<center>
			<nav aria-label="Page navigation">
				<ul class="pagination">
					<li>
						<a aria-label="Previous" rel="1" onclick='fjMostrarLista("<?= $gsModulo; ?>", "<?= $gsComponente; ?>", this.rel);' >
					...
					for ($i = 1; $i <= $objInstancia->atrPaginaFinal; $i++) {
						...
						?>
						<li class="<?= $Activo; ?> ">
							<a rel="<?= $i; ?>" onclick='console.log(this.rel); fjMostrarLista("<?= $gsModulo; ?>", "<?= $gsComponente; ?>", this.rel);' >
								<?= $i; ?>
							</a>
						</li>
						<?php
					...
					?>
					<li>
						<a aria-label="Next" rel="<?= ($objInstancia->atrPaginaFinal); ?>" onclick='fjMostrarLista("<?= $gsModulo; ?>", "<?= $gsComponente; ?>", this.rel);' >
