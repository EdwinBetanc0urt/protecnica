...
class clsRol extends Connection
{
	...
	function __construct($piPrivilegio = 2)
	{
		...
		$this->atrTablasRelacionadas = array(
			array(
				"tabla" => "tseg_ACCESO_ROL_d",
				"id" => "id_acceso",
				"estatus" => "estatus_acceso_rol"
			),
			array(
				"tabla" => "tseg_USUARIO_m",
				"id" => "id_usuario",
	...
	/** 
	 * función models Listar Rol, consulta en la base de datos según el termino de búsqueda, paginación y orden
	 ...
	 */
	function fmListarIndexRol($pcBusqueda = "")
	{

		$sql = "
			SELECT R.*
			FROM {$this->atrTabla} AS R

			WHERE
				R.{$this->atrEstatus} = 'activo' AND
				(R.{$this->atrNombre} LIKE '%{$this->atrFormulario["setBusqueda"]}%' OR
				R.{$this->atrId_P} LIKE '%{$this->atrFormulario["setBusqueda"]}%') "; //selecciona todo de la tabla
		if ($this->atrOrden != "")
