...
class clsCiudad extends Connection
{
	...
	function __construct($piPrivilegio = 2)
	{
		...
		$this->atrTablasRelacionadas = array(); //id dependiente o padre
	 ...
	 * @return object $tupla, resultado de consulta SQL o en caso contrario un FALSE.
	 */
	function fmListarIndexCiudad($pcBusqueda = "")
	{
		$sql = "
			SELECT C.*, E.*, P.*
			...
			INNER JOIN tglo_ESTADO AS E
				ON E.id_estado = C.id_estado_fk

			INNER JOIN tglo_PAIS AS P
				ON P.id_pais = E.id_pais_fk
			WHERE
				C.{$this->atrEstatus} = 'activo' AND
				(C.{$this->atrNombre} LIKE '%{$this->atrFormulario["setBusqueda"]}%' OR
				C.{$this->atrId_P} LIKE '%{$this->atrFormulario["setBusqueda"]}%') "; //selecciona todo de la tabla
		if ($this->atrOrden != "")
