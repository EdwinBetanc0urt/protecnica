...
class clsEstado extends Connection
{
	...
	function __construct($piPrivilegio = 2)
	{
		...
		$this->atrTablasRelacionadas = array(
			array(
				"tabla" => "tglo_MUNICIPIO",
				"id" => "id_municipio",
				"estatus" => "estatus_municipio"
			),
			array(
				"tabla" => "tglo_CIUDAD",
				"id" => "id_ciudad",
				"estatus" => "estatus_ciudad"
			)
		); //id dependiente o padre

		$this->atrId_Superior = "id_pais_fk" ; //id dependiente o padre
	...
	function fmInsertar()
	{
		try {
			...
			$tupla = parent::ejecutar(
				$sql,
				[
					$this->atrFormulario["ctxNombreEstado"],
					$this->atrFormulario["cmbPais1"]
				]
			);
	...
	function fmConsultar()
	{
		$sql = "
			...
			WHERE
				...
				{$this->atrNombre} = '{$this->atrFormulario["ctxNombreEstado"]}'
			LIMIT 1;";
	 ...
	 * @return object $tupla, resultado de consulta SQL o en caso contrario un FALSE.
	 */
	function fmListarIndexEstado($pcBusqueda = "")
	{

		$sql = "
			SELECT E.*, P.*
			FROM {$this->atrTabla} AS E

			INNER JOIN tglo_PAIS AS P
				ON P.id_pais = E.id_pais_fk

			WHERE
				E.{$this->atrEstatus} = 'activo' AND
				(E.{$this->atrNombre} LIKE '%{$this->atrFormulario["setBusqueda"]}%' OR
				E.{$this->atrId_P} LIKE '%{$this->atrFormulario["setBusqueda"]}%') "; //selecciona todo de la tabla
