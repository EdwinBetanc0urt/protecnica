...
class clsMunicipio extends Connection
{
	...
	function __construct($piPrivilegio = 2)
	{
		...
		$this->atrId_F = $this->atrClase . "_fk"; //clave foranea en otras tablas
		...
		$this->atrId_Superior = "id_estado_fk"; //id dependiente o padre
	...
	{
		$sql = "SELECT  M.*, E.nombre_estado AS estado FROM {$this->atrTabla} AS M
		INNER JOIN tglo_ESTADO AS E ON M.id_estado_fk = E.id_estado";
	 ...
	 * @return object $tupla, resultado de consulta SQL o en caso contrario un FALSE.
	 */
	function fmListarIndexMunicipio($pcBusqueda = "")
	{

		$sql = "
			SELECT Mun.*, Est.nombre_estado
			FROM {$this->atrTabla} AS Mun
			INNER JOIN tglo_ESTADO AS Est
				ON Mun.id_estado_fk = Est.id_estado
			WHERE
